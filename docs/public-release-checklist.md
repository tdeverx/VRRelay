...
- Verify `/api/v1/health`, the dashboard, and Helm app metadata report the semantic package version. Verify OCI labels, macOS DMG/app metadata, Windows installer metadata, and the build manifest carry the explicit product build identity where those formats support it. Verify the manifest records the full source commit and immutable OCI digest, then render the source-controlled Helm chart at that commit with the recorded relay digest.
