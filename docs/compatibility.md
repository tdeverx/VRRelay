...
H.265, AV1, copy codecs, fMP4 HLS, fragmented MP4, hardware-specific encoders, tone mapping, and subtitle burn-in are experiments that require retained matrix evidence before support can be claimed. Low-latency HLS, RTSP, HTTP MPEG-TS, HLS event playlists, mismatched HLS segment/container shapes, and passthrough policy profiles are not implemented profile outputs and cannot be saved as new profile revisions.
